...
while getopts "N:g:a:p:q:e:t:" opt; do
  case $opt in
    N) NAME="$OPTARG";;
    g) GENOME="$OPTARG";;
    a) GFFFILE="$OPTARG";;
    ...
    e) EVALUE="$OPTARG";;
    t) THREADS="$OPTARG";;
    *) echo "Usage: $0 -N name -g genome -a gff -p protein -q querydir [-e evalue] [-t threads]" >&2; exit 1;;
  esac
done
...
export PATH=$PATH:/path/to/blastp
export PATH=$PATH:/path/to/hmmsearch
...
GEMOMAP=/path/to/GeMoMa-1.7.1.jar
...
# Include the folder containing the FPDB databases 
QUERYDIR=${QUERYDIR}
